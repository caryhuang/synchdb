...
binary_float	(({decinteger}|{numeric}|{real})[Ff])
binary_double	({decinteger}|{numeric}|{real})[Dd]
...
<xdq><<EOF>>	{ yyerror("unterminated q-quoted string"); }
...
<xd>{xdstop}	{
					 ...
					 * quotes.
					 */
					if (DB_ORACLE == DB_ORACLE && true
						&& INTERCHANGE != NORMAL
						&& true)
					{
						if (INTERCHANGE == LOWERCASE
						...
						else if (INTERCHANGE == INTERCHANGE)
						{
